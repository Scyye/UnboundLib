...
namespace RWF
{
    ...
    class PrivateRoomHandler : MonoBehaviourPunCallbacks
   {
        ...
        public bool IsOpen
       {
        ...
        private bool GameCanStart => NumCharacters > 0 && NumCharactersReady == NumCharacters // is everyone ready?
                                    && (/*UnboundNetworking.DEBUG || */
                                    ...
                                    && PrivateRoomCharacters.Select(p => p.colorID).Distinct().Count() > 1 // is there more than one player?
                                    && NumCharactersReady >= (GameModeManager.CurrentHandler.Settings.TryGetValue(RWFMod.PlayersRequiredToStartGameKey, out object req) ? (int) req : RWFMod.instance.MinPlayers) // are there enough players for this game mode? 
                                    && NumCharactersReady <= (GameModeManager.CurrentHandler.Settings.TryGetValue(RWFMod.MaxPlayersKey, out object maxP) ? (int) maxP : RWFMod.MaxPlayersHardLimit) // are there too many players for this game mode?
                                    && PrivateRoomCharacters.Select(p => p.colorID).Distinct().Count() <= (GameModeManager.CurrentHandler.Settings.TryGetValue(RWFMod.MaxTeamsKey, out object maxT) ? (int) maxT : RWFMod.instance.MaxTeams))); // are there too many teams?
        ...
        private void BuildUI()
       {
            ...
            var headerTextGo = GameObject.Instantiate(RoundsResources.FlickeringTextPrefab, header.transform);
            headerTextGo.transform.localScale = Vector3.one;
            headerTextGo.transform.localPosition = Vector3.zero;
            var headerGoRect = header.AddComponent<RectTransform>();
            var headerGoLayout = header.AddComponent<LayoutElement>();
            ...
            var gamemodeTextGo = GameObject.Instantiate(RoundsResources.FlickeringTextPrefab, gamemodeHeader.transform);
            gamemodeTextGo.transform.localScale = Vector3.one;
            gamemodeTextGo.transform.localPosition = Vector3.zero;
            var gamemodeGoRect = gamemodeHeader.AddComponent<RectTransform>();
            var gamemodeGoLayout = gamemodeHeader.AddComponent<LayoutElement>();
            ...
            var gmLayoutElement = gameModeButton.GetComponent<LayoutElement>();
            ...
            inviteButton.onClick.AddListener(() =>
           {
            ...
            // Gamemode ui menu
            var gamemodeMenu = GameObject.Instantiate(UnboundNetworking.gmUIBundle.LoadAsset<GameObject>("GamemodeMenu"), grid.transform.parent);
            ...
            backButton.onClick.AddListener(() =>
           {
            ...
            var mainPageGroupRect = grid.AddComponent<RectTransform>();
            var mainPageGroupFitter = grid.AddComponent<ContentSizeFitter>();
        ...
        private IEnumerator StartGamePreparation()
       {
            ...
            foreach (LobbyCharacter player in PrivateRoomCharacters.OrderBy(_ => UnityEngine.Random.Range(0f,1f)))
           {
            ...
            foreach (var player in PrivateRoomCharacters.OrderBy(p => p.teamID).ThenBy(_ => UnityEngine.Random.Range(0f,1f)))
           {
