...
namespace Unbound.Gamemodes.Networking
{
    public static class NetworkingManager
    {
        ...
        private static readonly RaiseEventOptions raiseEventOptionsOthers = new RaiseEventOptions
        {
        ...
        private static readonly Dictionary<string, PhotonEvent> events = new Dictionary<string, PhotonEvent>();
        private static readonly Dictionary<Tuple<Type, string>, MethodInfo> rpcMethodCache = new Dictionary<Tuple<Type, string>, MethodInfo>();

        private static readonly byte ModEventCode = 69;
        ...
        public static void RaiseEvent(string eventName, RaiseEventOptions options, params object[] data)
        {
            ...
            List<object> allData = new List<object> { eventName };
        ...
        public static void RPC(Type targetType, string methodName, RaiseEventOptions options, SendOptions sendOptions, params object[] data)
        {
            ...
            {
                MethodInfo methodInfo = GetRPCMethod(targetType, methodName);
                if (methodInfo != null)
                {
            ...
            List<object> allData = new List<object> { targetType.AssemblyQualifiedName, methodName };
            allData.AddRange(data);
        ...
        public static void OnEvent(EventData photonEvent)
        {
            ...
            {
                if (data == null) return;
                Type type = Type.GetType((string) data[0]);
                if (type != null)
                {
                    MethodInfo methodInfo = GetRPCMethod(type, (string) data[1]);
                    if (methodInfo != null)
                    {
        ...
        private static MethodInfo GetRPCMethod(Type type, string methodName)
        {
            ...
            MethodInfo methodInfo = (from m in type.GetMethods(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.FlattenHierarchy)
